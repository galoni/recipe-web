...
from app.api.endpoints.recipes import create_recipe, read_recipes, delete_recipe
...
async def test_create_recipe(mock_db, mock_user, recipe_create_data):
    # Act
    result = await create_recipe(recipe=recipe_create_data, db=mock_db, current_user=mock_user)
