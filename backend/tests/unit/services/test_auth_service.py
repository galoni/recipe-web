...
async def test_authenticate_user_success():
    ...
    with patch(
        "app.services.auth_service.user_crud.authenticate", new_callable=AsyncMock
    ) as mock_auth:
...
async def test_register_new_user_success():
    ...
    with patch(
        "app.services.auth_service.user_crud.get_by_email", new_callable=AsyncMock
    ) as mock_get:
        mock_get.return_value = None
        with patch(
            "app.services.auth_service.user_crud.create", new_callable=AsyncMock
        ) as mock_create:
